...
# Set version number
#
VERSION=$(/usr/bin/echo 'Revision: 1.3' | /usr/bin/awk '{print $2}')
...
# shellcheck disable=SC2034 # Permit DEBUG variable
DEBUG=0
...
if [ -n "${QUERY_STRING}" ]; then 
  ...
  if echo "${QUERY_STRING}" | grep '=' >/dev/null ; then
    for Q in $(/usr/bin/echo "${QUERY_STRING}" | /usr/bin/tr ";&" "\012") ; do
      #
      ...
      #
      tmpvalue=${Q#*=}
      tmpvalue=$(/usr/bin/echo "${tmpvalue}" | \
...
function param {
  ...
  if [ $# -eq 1 ]; then
    ...
    value=$(/usr/bin/env | /usr/bin/grep "^FORM_${name}" | /usr/bin/sed -e 's/FORM_//' | /usr/bin/cut -d= -f2-)
  elif [ $# -gt 1 ]; then
    name=$1
    shift
    eval "export 'FORM_${name}=$*'"
  else
    value=$(/usr/bin/env | /usr/bin/grep '^FORM_' | /usr/bin/sed -e 's/FORM_//' | /usr/bin/cut -d= -f1)
  fi
  echo "${value}"
  unset name
  unset value
...
function cookie {
  ...
  if [ $# -eq 1 ]; then
    ...
    value=$(/usr/bin/env | /usr/bin/grep "^COOKIE_${name}" | /usr/bin/sed -e 's/COOKIE_//' | /usr/bin/cut -d= -f2-)
  elif [ $# -gt 1 ]; then
    name=$1
    shift
    eval "export 'COOKIE_${name}=$*'"
  else
    value=$(/usr/bin/env | /usr/bin/grep '^COOKIE_' | /usr/bin/sed -e 's/COOKIE_//' | /usr/bin/cut -d= -f1)
  fi
  echo "${value}"
  unset name
  unset value
...
parse_POST_params() {
  ...
  for ((i=0; i<${#parm[@]}; i+=2))
  do
      # shellcheck disable=2034 # permit POST_PARAMS
...
parse_GET_params() {
  ...
  for ((i=0; i<${#parm[@]}; i+=2))
  do  
      # shellcheck disable=2034 # permit GET_PARAMS
